...
beforeAll(async () => {
  ...
  const r = await request(app).post('/api/auth/login')
    .send({ email: 'cron@teste.com', senha: 'Senha@123' });
  token = r.body.token;
...
describe('[INTEGRATION] Cronômetro', () => {
  ...
  test('GET /api/cronometro/ativa - retorna sessão ativa', async () => {
    const res = await request(app).get('/api/cronometro/ativa')
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(200);
  });

  test('PATCH /api/cronometro/pausar - pausa sessão', async () => {
    const res = await request(app).patch('/api/cronometro/pausar')
      .set('Authorization', `Bearer ${token}`);
    expect([200, 400]).toContain(res.status);
  });

  test('PATCH /api/cronometro/retomar - retoma sessão', async () => {
